  ...
  <div class="wrap">
    ...
    <form method="post" action="options.php">
      ...
      <table class="form-table" role="presentation">
        ...
        <tr><th colspan="2"><h2><?php esc_html_e('Automated booking (Cal.com)', 'nirog-bhumi'); ?></h2><p class="description"><?php esc_html_e('Configure this once and the paid consultation flow becomes fully automatic: no manual WhatsApp payment handoff, no manual invoice trigger, no manual calendar link.', 'nirog-bhumi'); ?></p></th></tr>
        <tr>
          ...
          <td>
            ...
            <p class="description"><?php esc_html_e('The part of your Cal.com booking URL after cal.com/, e.g. gautam-khandelwal/consultation. Leave blank to keep the manual calendar page content instead.', 'nirog-bhumi'); ?></p>
          </td>
        </tr>
        <tr>
          <th scope="row"><label for="nirog-calcom-secret"><?php esc_html_e('Cal.com webhook secret', 'nirog-bhumi'); ?></label></th>
          <td>
            ...
            <p class="description">
              ...
              printf(
                ...
                esc_html__('In Cal.com go to Settings > Developer > Webhooks, add an endpoint pointing to %s, choose Booking Created/Rescheduled/Cancelled, set a secret, and paste the same secret here.', 'nirog-bhumi'),
...
/**
 * Receive Cal.com booking webhooks and write the confirmed slot, meeting
 * link and joining details straight into the matching consultation entry -
 * removing the need for anyone to type the appointment time into WordPress
 * by hand. Cal.com is told which entry a booking belongs to via the
 * metadata[nbEntryId] field passed into the embed above.
 */
...
function nirog_bhumi_install_invoice_sequence_table() {
  ...
  if (nirog_bhumi_invoice_sequence_table_exists()) {
    update_option('nirog_bhumi_invoice_sequence_version', '2');
    return true;
  }
  return false;
}
...
function nirog_bhumi_invoice_sequence_table_exists() {
  global $wpdb;
  $table = $wpdb->prefix . 'nb_invoice_sequences';
...
function nirog_bhumi_maybe_install_invoice_sequence_table() {
  if (get_option('nirog_bhumi_invoice_sequence_version') !== '2' || !nirog_bhumi_invoice_sequence_table_exists()) {
    return nirog_bhumi_install_invoice_sequence_table();
  }
  return true;
}
...
function nirog_bhumi_assign_sequential_invoice_number($post_id) {
  ...
  $lock_key = 'nirog_bhumi_invoice_lock_' . absint($post_id);
  $lock_acquired = add_option($lock_key, time(), '', false);
  if (!$lock_acquired && (int) get_option($lock_key) < time() - 120) {
  ...
  try {
    ...
    delete_post_meta($post_id, '_nb_test_invoice');
    return $invoice_number;
  } finally {
    delete_option($lock_key);
  }
...
function nirog_bhumi_render_consultation_booking_metabox($post) {
  ...
  $invoice_error = (string) get_post_meta($post->ID, 'invoice_error', true);
  $invoice_action_url = wp_nonce_url(admin_url('admin-post.php?action=nirog_retry_invoice&post_id=' . $post->ID), 'nirog_retry_invoice_' . $post->ID);
  ?>
...
function nirog_bhumi_send_consultation_invoice($post_id) {
  update_post_meta($post_id, 'invoice_generation_attempted_at', current_time('mysql'));
