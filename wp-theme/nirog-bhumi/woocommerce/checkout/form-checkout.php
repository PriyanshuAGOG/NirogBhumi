...
<?php if (!$checkout->is_registration_enabled() && $checkout->is_registration_required() && !is_user_logged_in()) : ?>
  ...
  <?php return; ?>
<?php endif; ?>
<form name="checkout" method="post" class="checkout woocommerce-checkout commerce-checkout" action="<?php echo esc_url(wc_get_checkout_url()); ?>" enctype="multipart/form-data">
  <div class="checkout-head">
    <p class="eyebrow"><?php esc_html_e('Checkout', 'nirog-bhumi'); ?></p>
    <h1><?php esc_html_e('Secure checkout.', 'nirog-bhumi'); ?></h1>
    <p><?php esc_html_e('Confirm contact, billing, delivery, order notes and payment details before placing your Nirog Bhumi order.', 'nirog-bhumi'); ?></p>
  </div>
  <div class="checkout-flow">
    <?php if ($checkout->get_checkout_fields()) : ?>
      <?php do_action('woocommerce_checkout_before_customer_details'); ?>
      <section class="checkout-panel" id="customer_details">
        <div class="panel-title"><span>01</span><h2><?php esc_html_e('Billing details', 'nirog-bhumi'); ?></h2></div>
